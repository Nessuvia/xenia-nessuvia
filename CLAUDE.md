...
- **Style checks** are the second kind of rule: code, not config. `core/agent/lintRules.ts` holds
  ...
  `quality/vadData.ts`, generated by `scripts/buildVad.mjs` and loaded as its own chunk by
  `loadVad` (async) before `arousalOf` or `vadOf` (sync) returns anything. The chunk stays out of precache. A new check is one file plus one registry entry.
