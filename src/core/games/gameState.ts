...
export interface StateBlockContext {
   ...
   *
   * The block is built from the state *after* the move: a card handed over is already gone from
  ...
  for (const event of events) {
    switch (event.kind) {
      ...
      case 'give':
        ...
        break
      case 'draw':
        // The other side's draw is face down: its rank stays out of the line.
