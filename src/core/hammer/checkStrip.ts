...
import type { Rule } from '../agent/rules.ts'
...
const s1 = stripWith('She runs with a graceful elegance.', [r1], tagger)
...
const pv = previewStrips('She runs with a graceful elegance.', [r1], tagger)
...
const pvEnd = previewStrips('He took the glass with a practiced hand.', [r1], tagger)
...
const sOff = stripWith('She runs very quickly.', [rOff], tagger)
...
const sBad = stripWith('She runs fast.', [rBad], tagger)
...
const rAdj = rule('[adj]')
const fenced = stripWith('She runs `quick` fast.', [rAdj], tagger)
...
const rRep = rule('with a [adj] [noun]', { action: 'swap', replacement: '$4' })
const sRep = stripWith('She runs with a graceful elegance.', [rRep], tagger)
assert.equal(sRep.text, 'She runs elegance.', JSON.stringify(sRep.text))
// $0 echoes the whole match; a literal + ref mix composes.
const rRep0 = rule('with a [adj] [noun]', { action: 'swap', replacement: 'with $4' })
const sRep0 = stripWith('She runs with a graceful elegance.', [rRep0], tagger)
...
const pvRep = previewStrips('She runs with a graceful elegance.', [rRep], tagger)
...
for (const sample of samples) {
  const { text } = stripWith(sample, [r1, rule('[adj] and [adj]'), rule('not just [noun], but [noun]')], tagger)
...
stripWith(original, [r1], tagger)
assert.equal(original, 'She runs with a graceful elegance.')

// Default tagger path works (memoized).
const sDefault = stripText('She runs with a graceful elegance.', [r1])
...
const rFlag = rule('[adv] [adj]', { action: 'rewrite' })
const flagSource = 'She runs very quickly and looks quietly furious.'
assert.equal(stripWith(flagSource, [rFlag], tagger).text, flagSource)
const flags = findFlags(flagSource, [rFlag], tagger)
...
const flagsMulti = findFlags(flagSource, [rFlag, rule('[adj] and [adj]', { action: 'rewrite' })], tagger)
...
assert.equal(findFlags(flagSource, [rule('[adv] [adj]', { action: 'rewrite', enabled: false })], tagger).length, 0)

// A strip rule and a flag rule in one set: the strip applies, the flag does not become an edit.
const mixed = stripWith('She runs with a graceful elegance very quickly.', [r1, rFlag], tagger)
