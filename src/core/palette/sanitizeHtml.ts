 ...
 *
 * A `<template>` is the parser on purpose. Regex over HTML misses nested and quoted forms: that's
 ...
 * at, and there's only ever one image per slot: one name is enough. Bare `image` counts too: the
...
function clean(node: Node, invalid: Set<string>, imageSrc: string): Node[] {
  ...
  for (const attr of Array.from(el.attributes)) {
    if (attr.name === 'src' && isBackgroundImageRef(attr.value)) {
      // The slot's own image. Left as written when there's none: the panel's validation pass
