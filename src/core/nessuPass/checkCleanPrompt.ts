...
// --- standing rules in the prompt -----------------------------------------
{
  ...
  const both = buildCleanPrompt('x', [note({ message: 'Used before.' })], '', standing)[1].content
...
// --- the prompt -----------------------------------------------------------
{
  ...
  const msgs = buildCleanPrompt(text, [note({ slice: 'a mixture of fear and desire' })], '')
...
{
  const msgs = buildCleanPrompt('x', [note(), note({ message: 'Second.', fix: 'better' })], '')
...
{
  const msgs = buildCleanPrompt('x', [], 'tighten dialogue')
...
{
  const body = buildCleanPrompt('x', [note({ slice: '   ' })], '')[1].content
